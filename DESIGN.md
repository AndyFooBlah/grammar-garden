...
**Income:** captured light × sun strength (default 0.6), in full on sunny ticks and at 50% during rain.
...
- **Weather per zone.** Each zone alternates rain and sun spells. Two settings shape it: the zone's **rain fraction** (average share of rainy ticks; left 10% and right 80% by default, so one side is a near-desert and the other a rainforest) and a shared **cycle length** (40 ticks by default). A spell's length is its share of the cycle with ±25% jitter, so the two zones drift out of step. Cycle length 2 alternates every tick; 200 gives a hundred of each. Growth happens during a plant's own zone's rain.
- **Soil moisture** (0–100%) rises linearly while it rains (15% per tick) and falls linearly in the sun (2% per tick). The dirt colour goes from pale and dry to dark and wet, and a badge over each zone shows the weather, ticks to go, and soil percentage.
- **Water need.** Each plant needs the soil to be at least as wet as 1% × (green segments + 2 × flowers + 0.1 × wood segments), capped at 100%. Green and flowers are thirsty; wood hardly at all (so woody plants can live where leafy ones cannot). The inspector shows the need next to the current soil.
- **Thirst.** When the soil is drier than the need, the plant loses 6 energy per tick for every 10% of the shortfall (raised from 3 after playtesting: dry soil should really hurt). Big leafy plants in dry soil die within a few ticks; a small deficit is a slow drain. **Died of thirst** is its own death reason and statistic.
- **Bugs** only work where the sun is out. If one zone rains, they fly to the other; if both rain they shelter at the field's edges as before. When both zones are sunny, bugs favour the zone where the sun came out most recently (flowers freshly open after rain), so a rainy zone gets the full pollinator force during its short sunny spells. Bugs fly 400 px per tick so they can cross a zone in a few ticks.
...
- **Kinds of plants (v4).** Every grown plant is sorted into an everyday kind from three measurements: height, width relative to height, and the share of its segments that are wood. Seeds have nothing grown yet; **trees** are at least 25% wood and 100 px tall; **shrubs** are woody but shorter; **bushes** are green and at least 80% as wide as they are tall; **grass** is green, narrow and under 60 px; anything else green is a **herb**. The card shows the current counts and a stacked area chart of the mix sampled every 5 ticks (the last 600 samples, saved with the garden), so a shift from grass to trees or from bushes to shrubs on the dry side is visible at a glance. The inspector and the families list use the same words.
