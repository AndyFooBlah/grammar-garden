...
*Working title. Status: design for review, not yet implemented.*
...
**Design decision to confirm:** `[` and `]` are not in your original college grammar. Without them, the only way to make a side branch is to walk out with `f` and back with `b`, which retraces the same line. Brackets make bushy, tree-like shapes easy and are the standard L-system convention. The editor will keep them balanced automatically (an unmatched `]` is ignored; unclosed branches are closed at the end).
...
Limits (world settings): a maximum number of growth steps per plant, and a maximum string length (default 4,000 symbols) after which the plant simply stops growing. This keeps the browser responsive when a kid types `A=AAAA`.

### 2.3 Drawing (the turtle)

The turtle starts at the plant's seed position on the ground line, pointing straight up, green pen, base step size (default 10 px). Every `f`/`b` produces a **segment** with a color and a parent segment (the one the turtle was standing on when it started), so the plant is a tree of segments. Flowers attach to the segment where they are placed.
...
- **Health**: 0–100, shown as a green bar in the inspector. Health drops each tick a plant is **shaded** (see below) and drops steadily once a plant is older than the **lifespan** setting. At 0 the plant dies of old age or shade: a soft rustle, it fades, and its space is freed.
- **Shaded**: a plant is shaded if a taller neighbor's canopy (its horizontal extent, expanded by a "shade margin" setting) covers the plant's root position. Shaded plants skip growth ticks and lose health. This is what makes room matter: a seed dropped under a big tree never gets going.
- **Room**: the world has a max plant count and a minimum seed spacing. Bugs won't drop a seed if there is no room; the seed is simply lost (a quiet "plip" doesn't play, so the kid learns that silence means the field is full).
...
- **DNA editor.** A large-font textarea. As you type, the close-up redraws at the plant's current growth step and shows ✔ or the specific problem. **Apply** replaces the plant's DNA and regrows it from seed in place. **Clone to seed** drops a new seed with a copy of the DNA at a free spot (a great way to say "I like this one, make more"). **✕** removes the plant. **Surprise me** fills the editor with a random recipe.
- **World settings.** Sliders with plain labels: tick length, sun length, rain length, plant lifespan, bees, butterflies, max plants, seed spacing, max growth steps, turn angle, step size, max unsupported load, mutation rate, shade margin, allow self-pollination. Changing a geometric setting (angle, step, load) regrows every plant so the effect is instantly visible.
...
| Seed planted | Soft "plip": a short sine blip sliding down in pitch |
...
```json
{
  ...
  "plants": [
    { "id": 17, "name": "Zippy", "dna": "A=wfB;B=f[lC][rC]fB;C=gfy",
      "x": 312, "steps": 6, "age": 40, "health": 88, "stage": "mature",
      "generation": 3, "parents": [4, 9] }
  ],
  "bugs": [ { "kind": "bee", "x": 100, "y": 60, "carrying": 17 } ]
}
```
...
- **Modules** (each small and unit-tested with Vitest):
  ...
  - `render.ts` — field and inspector drawing.
...
1. Add `[` `]` branch brackets to the grammar (recommended).
2. Flowers are dots placed by `y`/`p`, rather than pen colors for lines.
3. The wood rule is load-based: a green segment may hold up at most N step-lengths of plant above it.
4. Growth happens only during rain; bugs fly only in sun.
...
8. Name: "Grammar Garden" (alternatives welcome).
